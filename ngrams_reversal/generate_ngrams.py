...
def print_log(log_str):
    with open("ng_log.txt", "a") as myfile:
        myfile.write(strftime("%Y-%m-%d %H:%M:%S") + '\t' + log_str + '\n')


def process_dir(data_dir, target_dir, MIN_N_GRAM, MAX_N_GRAM, b_verbose=False, b_size=None):
    """
    ...
    # target_dir = os.path.join(data_dir, 'ngrams')
    ...
    for case_file in case_files:
        ...
        if b_verbose:
            print_log(str(progress / (0.01 * total_count)) + ' % Complete')
