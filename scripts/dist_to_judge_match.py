...
df = pd.read_csv('csv/District_sorted.csv')
...
for index, row in df.iterrows():
    ...
    if judge_name != "":
        ...
        matched_judge_names = judges_df[judges_df.apply(lambda dfj: fuzz.token_sort_ratio(dfj['songername'], judge_name) > 0.95)]

        if ~(matched_judge_names.empty):

            print("Exact Match", matched_judge_names['songername'], judge_name)
            continue

        matched_judge_names = judges_df[judges_df.apply(lambda dfj: fuzz.token_sort_ratio(dfj['judgelastname'], judge_name) > 0.95)]

        if matched_judge_names.shape[0] != 1:
            continue
        ...
        for i, j_rows in matched_judge_names.iterrows():
            ...
            if code_to_state[row_j_df.SeatState] in row.courtname:
                ...
                if match_score == 2:
                    ...
                    print("\n\n")
                    print("CSV Name: " + row.judges + "\nLastname: " + j_rows['judgelastname'])

#         if pd.matched_judge_names
#             match_score = 0
#
